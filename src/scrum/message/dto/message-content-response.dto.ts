...
export class MessageContentResponseDTO {
  constructor(ID: string, UUID: string, content: MessageContentDTO) {
    this.ID = ID;
    this.UUID = UUID;
  ...
  @ApiProperty({
    description: 'ID is Unique identifier for ' + MessageContentResponseDTO.name,
    example: 'john.doe',
  })
  ...
  @ApiProperty({
    description:
      ...
      ' for  ' +
      MessageContentResponseDTO.name,
