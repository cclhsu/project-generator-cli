...
import { UpdateMessageMetadataRequestDTO } from '../dto';
...
@SubCommand({
  ...
  description: 'A command to update a message metadata',
})
export class UpdateMessageMetadataCommand extends CommandRunner {
  private readonly logger = new Logger(UpdateMessageMetadataCommand.name);
  constructor(
  ...
  async run(
    ...
    const updateMessageMetadataRequestDTO: UpdateMessageMetadataRequestDTO = {
      // ID: message.ID,
      UUID: message.UUID,
    ...
    try {
      this.logger.verbose(
        JSON.stringify(updateMessageMetadataRequestDTO, null, 2),
      );
      await this.messageService.updateMessageMetadata(
        updateMessageMetadataRequestDTO.UUID,
