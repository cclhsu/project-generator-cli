...
import { UpdateMessageContentRequestDTO } from '../dto';
...
@SubCommand({
  name: 'update-content',
  description: 'A command to update a message content content',
})
export class UpdateMessageContentCommand extends CommandRunner {
  private readonly logger = new Logger(UpdateMessageContentCommand.name);
  constructor(
  ...
  async run(
    ...
    const updateMessageContentRequestDTO: UpdateMessageContentRequestDTO = {
      // ID: message.ID,
      UUID: message.UUID,
    ...
    try {
      this.logger.verbose(
        JSON.stringify(updateMessageContentRequestDTO, null, 2),
      );
      await this.messageService.updateMessageContent(
        updateMessageContentRequestDTO.UUID,
