...
export class ListTaskIdsUUIDsStatusesCommand extends CommandRunner {
  private readonly logger = new Logger(ListTaskIdsUUIDsStatusesCommand.name);
  constructor(
  ...
  async run(
    ...
    try {
      const task: TaskIdUuidDTO[] =
        await this.taskService.ListTaskIdsUUIDsStatues();
