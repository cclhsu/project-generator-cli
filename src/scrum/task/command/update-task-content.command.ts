...
import { UpdateTaskContentRequestDTO } from '../dto';
...
@SubCommand({
  name: 'update-content',
  description: 'A command to update a task content',
})
export class UpdateTaskContentCommand extends CommandRunner {
  private readonly logger = new Logger(UpdateTaskContentCommand.name);
  constructor(
  ...
  async run(
    ...
    const updateTaskContentRequestDTO: UpdateTaskContentRequestDTO = {
      // ID: task.ID,
      UUID: task.UUID,
    ...
    try {
      this.logger.verbose(JSON.stringify(updateTaskContentRequestDTO, null, 2));
      await this.taskService.updateTaskContent(
        updateTaskContentRequestDTO.UUID,
