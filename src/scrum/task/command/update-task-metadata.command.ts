...
import { UpdateTaskMetadataRequestDTO } from '../dto';
...
@SubCommand({
  ...
  description: 'A command to update a task metadata',
})
export class UpdateTaskMetadataCommand extends CommandRunner {
  private readonly logger = new Logger(UpdateTaskMetadataCommand.name);
  constructor(
  ...
  async run(
    ...
    const updateTaskMetadataRequestDTO: UpdateTaskMetadataRequestDTO = {
      // ID: task.ID,
      UUID: task.UUID,
    ...
    try {
      this.logger.verbose(
        JSON.stringify(updateTaskMetadataRequestDTO, null, 2),
      );
      await this.taskService.updateTaskMetadata(
        updateTaskMetadataRequestDTO.UUID,
