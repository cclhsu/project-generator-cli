...
import { UpdateMetricContentRequestDTO } from '../dto';
...
@SubCommand({
  name: 'update-content',
  description: 'A command to update a metric content content',
})
export class UpdateMetricContentCommand extends CommandRunner {
  private readonly logger = new Logger(UpdateMetricContentCommand.name);
  constructor(
  ...
  async run(
    ...
    const updateMetricContentRequestDTO: UpdateMetricContentRequestDTO = {
      // ID: metric.ID,
      UUID: metric.UUID,
    ...
    try {
      this.logger.verbose(
        JSON.stringify(updateMetricContentRequestDTO, null, 2),
      );
      await this.metricService.updateMetricContent(
        updateMetricContentRequestDTO.UUID,
