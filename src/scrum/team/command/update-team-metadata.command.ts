...
import { UpdateTeamMetadataRequestDTO } from '../dto';
...
@SubCommand({
  ...
  description: 'A command to update a team metadata',
})
export class UpdateTeamMetadataCommand extends CommandRunner {
  private readonly logger = new Logger(UpdateTeamMetadataCommand.name);
  constructor(
  ...
  async run(
    ...
    try {
      this.logger.verbose(
        JSON.stringify(updateTeamMetadataRequestDTO, null, 2),
      );
      await this.teamService.updateTeamMetadata(
        updateTeamMetadataRequestDTO.UUID,
  ...
  parseMembers(val: string): IdUuidDTO[] {
    ...
    if (!isValidIdUuidArray(items)) {
      throw new Error(
        UpdateTeamMetadataCommand.name +
