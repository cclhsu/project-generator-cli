...
import { UpdateProjectContentRequestDTO } from '../dto';
...
@SubCommand({
  name: 'update-content',
  description: 'A command to update a project content',
})
export class UpdateProjectContentCommand extends CommandRunner {
  private readonly logger = new Logger(UpdateProjectContentCommand.name);
  constructor(
  ...
  async run(
    ...
    const updateProjectContentRequestDTO: UpdateProjectContentRequestDTO = {
      // ID: project.ID,
      UUID: project.UUID,
    ...
    try {
      this.logger.verbose(
        JSON.stringify(updateProjectContentRequestDTO, null, 2),
      );
      await this.projectService.updateProjectContent(
        updateProjectContentRequestDTO.UUID,
  ...
  parseIterations(val: string): IdUuidStatusDTO[] {
    ...
    if (!isValidIdUuidStatusArray(items)) {
      throw new Error(
        UpdateProjectContentCommand.name +
