...
@SubCommand({
  name: 'update',
  description: 'A command to update a project',
})
export class UpdateProjectCommand extends CommandRunner {
  private readonly logger = new Logger(UpdateProjectCommand.name);
  constructor(
  ...
  async run(
    ...
    try {
      this.logger.verbose(JSON.stringify(updateProjectRequestDTO, null, 2));
      await this.projectService.updateProject(
        updateProjectRequestDTO.UUID,
