...
@SubCommand({
  ...
  description: 'A command to update a user metadata',
})
export class UpdateUserMetadataCommand extends CommandRunner {
  private readonly logger = new Logger(UpdateUserMetadataCommand.name);
  constructor(
  ...
  async run(
    ...
    try {
      this.logger.verbose(
        JSON.stringify(updateUserMetadataRequestDTO, null, 2),
      );
      await this.userService.updateUserMetadata(
        updateUserMetadataRequestDTO.UUID,
        updateUserMetadataRequestDTO,
      );
    } catch (error: any) {
...
// npm run build
// nestjs build
// node ./dist/cmd.main user update-metadata --help
// node ./dist/cmd.main user update-metadata
// node ./dist/cmd.main user update-metadata --uuid 15a955b3-bc5f-4e33-a29d-eb0738b15a9f --id john.doe --name 'John Doe' --startDate '2023-01-01T00:00:00.000Z' --endDate '2023-12-31T00:00:00.000Z' --updatedBy john.kent
// node ./dist/cmd.main user update-metadata --uuid f7f7e6eb-13be-4c22-b52a-2bcba29c74a7 --id jane.doe --name 'Jane Doe' --startDate '2023-01-01T00:00:00.000Z' --endDate '2023-12-31T00:00:00.000Z' --updatedBy jane.kent --startedAt '2023-01-01T00:00:00.000Z' --startedBy 'john.doe' --completedAt '2023-12-31T00:00:00.000Z' --completedBy 'jane.doe'
