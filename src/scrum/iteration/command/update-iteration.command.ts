...
export class UpdateIterationCommand extends CommandRunner {
  ...
  async run(
    ...
    try {
      this.logger.verbose(JSON.stringify(updateIterationRequestDTO, null, 2));
      await this.iterationService.updateIteration(
  ...
  //     throw new Error(UpdateIterationCommand.name + ': Invalid user ID, UUID, Status in the list: ' + val);
  //   }
  //   return items;
  // }
