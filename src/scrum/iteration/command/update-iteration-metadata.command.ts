...
import { UpdateIterationMetadataRequestDTO } from '../dto';
...
@SubCommand({
  ...
  description: 'A command to update a iteration metadata',
})
export class UpdateIterationMetadataCommand extends CommandRunner {
  private readonly logger = new Logger(UpdateIterationMetadataCommand.name);
  constructor(
  ...
  async run(
    ...
    const updateIterationMetadataRequestDTO: UpdateIterationMetadataRequestDTO =
      {
        // ID: iteration.ID,
        UUID: iteration.UUID,
    ...
    try {
      this.logger.verbose(
        JSON.stringify(updateIterationMetadataRequestDTO, null, 2),
      );
      await this.iterationService.updateIterationMetadata(
        updateIterationMetadataRequestDTO.UUID,
  ...
  parseTasks(val: string): IdUuidStatusDTO[] {
    ...
    if (!isValidIdUuidStatusArray(items)) {
      throw new Error(
        UpdateIterationMetadataCommand.name +
  ...
  //     throw new Error(UpdateIterationMetadataCommand.name + ': Invalid user ID, UUID, Status in the list: ' + val);
  //   }
  //   return items;
  // }
