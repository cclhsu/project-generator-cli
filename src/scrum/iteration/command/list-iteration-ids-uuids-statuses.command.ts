...
@Injectable()
@SubCommand({
  name: 'list-iteration-ids-uuids-statuses',
  description: 'A command to list iterations',
  options: { isDefault: true },
})
export class ListIterationIdsUUIDsStatusesCommand extends CommandRunner {
  private readonly logger = new Logger(
    ListIterationIdsUUIDsStatusesCommand.name,
  );
  constructor(
  ...
  async run(
    ...
    try {
      const iteration: IterationIdUuidDTO[] =
        await this.iterationService.listIterationIdsUUIDsStatuses();
