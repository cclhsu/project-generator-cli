...
@SubCommand({
  name: 'run',
  description: 'A command to run a typescript project',
  options: { isDefault: true },
})
export class RunTypescriptCommand extends CommandRunner {
  private readonly logger = new Logger(RunTypescriptCommand.name);
  constructor(
  ...
  async run(
    ...
    this.logger.debug('>>> Running typescript');
    ...
    try {
      await this.typescriptService.run(projectCommandOptionsDTO);
    } catch (error: any) {
...
// npm run build
// nestjs build
// node ./dist/cmd.main typescript run --help
// node ./dist/cmd.main typescript run -s src -g github -l typescript -t library -e team -u user -n project -m module
