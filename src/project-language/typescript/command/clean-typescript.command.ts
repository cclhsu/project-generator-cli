...
@SubCommand({
  name: 'clean',
  description: 'A command to clean a typescript project',
})
export class CleanTypescriptCommand extends CommandRunner {
  private readonly logger = new Logger(CleanTypescriptCommand.name);
  constructor(
  ...
  async run(
    ...
    this.logger.debug('>>> Cleaning typescript');
    ...
    try {
      await this.typescriptService.clean(projectCommandOptionsDTO);
    } catch (error: any) {
...
// npm run build
// nestjs build
// node ./dist/cmd.main typescript clean --help
// node ./dist/cmd.main typescript clean -s src -g github -l typescript -t library -e team -u user -n project -m module
