...
@SubCommand({
  name: 'init',
  description: 'A command to init a typescript project',
})
export class InitTypescriptCommand extends CommandRunner {
  private readonly logger = new Logger(InitTypescriptCommand.name);
  constructor(
  ...
  async run(
    ...
    this.logger.debug('>>> initializing typescript');
...
// npm run build
// nestjs build
// node ./dist/cmd.main typescript init --help
// node ./dist/cmd.main typescript init -s src -g github -l typescript -t library -e team -u user -n project -m module
