...
@SubCommand({
  name: 'build',
  description: 'A command to build a typescript project',
})
export class BuildTypescriptCommand extends CommandRunner {
  private readonly logger = new Logger(BuildTypescriptCommand.name);
  constructor(
  ...
  async run(
    ...
    this.logger.debug('>>> Building typescript');
    ...
    try {
      await this.typescriptService.init(projectCommandOptionsDTO);
    } catch (error: any) {
...
// npm run build
// nestjs build
// node ./dist/cmd.main typescript build --help
// node ./dist/cmd.main typescript build -s src -g github -l typescript -t library -e team -u user -n project -m module
