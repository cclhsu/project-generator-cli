...
@SubCommand({
  name: 'clean',
  description: 'A command to clean a python3 project',
})
export class CleanPython3Command extends CommandRunner {
  private readonly logger = new Logger(CleanPython3Command.name);
  constructor(
  ...
  async run(
    ...
    this.logger.debug('>>> Cleaning python3');
    ...
    try {
      await this.python3Service.clean(projectCommandOptionsDTO);
    } catch (error: any) {
...
// npm run build
// nestjs build
// node ./dist/cmd.main python3 clean --help
// node ./dist/cmd.main python3 clean -s src -g github -l python3 -t library -e team -u user -n project -m module
