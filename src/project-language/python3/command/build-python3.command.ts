...
@SubCommand({
  name: 'build',
  description: 'A command to build a python3 project',
})
export class BuildPython3Command extends CommandRunner {
  private readonly logger = new Logger(BuildPython3Command.name);
  constructor(
  ...
  async run(
    ...
    this.logger.debug('>>> Building python3');
    ...
    try {
      await this.python3Service.build(projectCommandOptionsDTO);
    } catch (error: any) {
...
// npm run build
// nestjs build
// node ./dist/cmd.main python3 build --help
// node ./dist/cmd.main python3 build -s src -g github -l python3 -t library -e team -u user -n project -m module
