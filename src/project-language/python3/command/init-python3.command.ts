...
@SubCommand({
  name: 'init',
  description: 'A command to init a python3 project',
})
export class InitPython3Command extends CommandRunner {
  private readonly logger = new Logger(InitPython3Command.name);
  constructor(
  ...
  async run(
    ...
    this.logger.debug('>>> initializing python3');
    ...
    try {
      await this.python3Service.init(projectCommandOptionsDTO);
    } catch (error: any) {
...
// npm run build
// nestjs build
// node ./dist/cmd.main python3 init --help
// node ./dist/cmd.main python3 init -s src -g github -l python3 -t library -e team -u user -n project -m module
