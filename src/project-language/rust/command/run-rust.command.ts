...
@SubCommand({
  name: 'run',
  description: 'A command to run a rust project',
  options: { isDefault: true },
})
export class RunRustCommand extends CommandRunner {
  private readonly logger = new Logger(RunRustCommand.name);
  constructor(
  ...
  async run(
    ...
    this.logger.debug('>>> Running rust');
    ...
    try {
      await this.rustService.run(projectCommandOptionsDTO);
    } catch (error: any) {
...
// npm run build
// nestjs build
// node ./dist/cmd.main rust run --help
// node ./dist/cmd.main rust run -s src -g github -l rust -t library -e team -u user -n project -m module
