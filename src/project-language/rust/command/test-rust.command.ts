...
@SubCommand({
  name: 'test',
  description: 'A command to test a rust project',
})
export class TestRustCommand extends CommandRunner {
  private readonly logger = new Logger(TestRustCommand.name);
  constructor(
  ...
  async run(
    ...
    this.logger.debug('>>> Testing rust');
    ...
    try {
      await this.rustService.test(projectCommandOptionsDTO);
    } catch (error: any) {
...
// npm run build
// nestjs build
// node ./dist/cmd.main rust test --help
// node ./dist/cmd.main rust test -s src -g github -l rust -t library -e team -u user -n project -m module
