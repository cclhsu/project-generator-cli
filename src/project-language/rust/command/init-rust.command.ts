...
@SubCommand({
  name: 'init',
  description: 'A command to init a rust project',
})
export class InitRustCommand extends CommandRunner {
  private readonly logger = new Logger(InitRustCommand.name);
  constructor(
  ...
  async run(
    ...
    this.logger.debug('>>> initializing rust');
    ...
    try {
      await this.rustService.init(projectCommandOptionsDTO);
    } catch (error: any) {
...
// npm run build
// nestjs build
// node ./dist/cmd.main rust init --help
// node ./dist/cmd.main rust init -s src -g github -l rust -t library -e team -u user -n project -m module
