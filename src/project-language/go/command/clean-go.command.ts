...
@SubCommand({
  name: 'clean',
  description: 'A command to clean a go project',
})
export class CleanGoCommand extends CommandRunner {
  private readonly logger = new Logger(CleanGoCommand.name);
  constructor(
  ...
  async run(
    ...
    try {
      await this.goService.clean(projectCommandOptionsDTO);
    } catch (error: any) {
...
// npm run build
// nestjs build
// node ./dist/cmd.main go clean --help
// node ./dist/cmd.main go clean -s src -g github -l go -t library -e team -u user -n project -m module
