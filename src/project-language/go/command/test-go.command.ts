...
@SubCommand({
  name: 'test',
  description: 'A command to test a go project',
})
export class TestGoCommand extends CommandRunner {
  private readonly logger = new Logger(TestGoCommand.name);
  constructor(
...
// npm run build
// nestjs build
// node ./dist/cmd.main go test --help
// node ./dist/cmd.main go test -s src -g github -l go -t library -e team -u user -n project -m module
