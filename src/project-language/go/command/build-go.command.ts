...
@SubCommand({
  name: 'build',
  description: 'A command to build a go project',
})
export class BuildGoCommand extends CommandRunner {
  private readonly logger = new Logger(BuildGoCommand.name);
  constructor(
  ...
  async run(
    ...
    try {
      await this.goService.build(projectCommandOptionsDTO);
    } catch (error: any) {
...
// npm run build
// nestjs build
// node ./dist/cmd.main go build --help
// node ./dist/cmd.main go build -s src -g github -l go -t library -e team -u user -n project -m module
